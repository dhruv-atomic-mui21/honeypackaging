...
// 1. Force HTTPS in Production & Security Headers
app.use((req, res, next) => {
  ...
  // Force HTTPS if behind reverse proxy in production
  if (
...
// In-memory rate limiting for contact form (max 5 requests per 15 minutes per IP)
...
function checkRateLimit(ip) {
  ...
  const validTimestamps = record.filter((t) => now - t < RATE_LIMIT_WINDOW);
...
// Clean up stale rate limit entries hourly
setInterval(() => {
  const now = Date.now();
...
app.get('/api/health', (_req, res) => {
  res.json({
    status: 'ok',
    app: 'Honey Packaging API',
    uptime: process.uptime(),
    timestamp: new Date().toISOString(),
  });
});

// Quote submission endpoint with anti-spam & rate limiting
app.post('/api/send-quote', async (req, res) => {
  ...
  if (!checkRateLimit(clientIp)) {
    return res.status(429).json({
      error: 'Too many requests submitted from this network. Please call us directly at +91 99099 22785.',
    });
  }
  ...
  if (hp_trap || website_url) {
    console.warn(`[AntiSpam] Blocked automated bot submission from IP ${clientIp}`);
    // Return 200 to fool bot without sending email
    return res.json({ success: true, note: 'ok' });
  }
